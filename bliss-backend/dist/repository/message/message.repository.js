...
export async function getBookingStatusFromDb(userId, serviceId) {
    try {
        await dbConnect();
...
export async function cancelBookingFromDb(userId, requestId) {
    try {
        await dbConnect();
...
export async function createBookingMessageInDb(bookingData) {
    try {
        await dbConnect();
        ...
        const newMessage = new MessageData({
            user: {
                id: booker.clerkId,
                name: bookerIsVendor ? booker.ownerName : booker.name,
                email: bookerIsVendor ? booker.ownerEmail : booker.email,
                phone: bookerIsVendor ? booker.owner_contactNo?.[0] : booker.phone,
...
export async function getVendorBookingRequestsFromDb(userId, options) {
    try {
        await dbConnect();
...
export async function updateVendorBookingRequestStatusFromDb(userId, requestId, status) {
    try {
        await dbConnect();
...
export async function getUserBookingRequestsFromDb(userId, options) {
    try {
        await dbConnect();
        const { status, limit = 50, page = 1 } = options;
        const query = {
