 ...
 */
export async function cleanupImages(publicIds) {
    try {
        const validPublicIds = publicIds.filter((id) => id && typeof id === 'string' && id.trim().length > 0);
        ...
        for (let i = 0; i < validPublicIds.length; i += batchSize) {
            ...
            const deletionResults = await Promise.allSettled(batch.map(async (publicId) => {
                try {
