...
export const cancelBooking = async (req, res, next) => {
    try {
        ...
        if (status !== 'cancelled') {
            throw new AppError('Invalid status. Only cancellation allowed from user side.', 400);
        }
 ...
 */
export const createBookingMessage = async (req, res, next) => {
    try {
 ...
 */
export const getVendorBookingRequests = async (req, res, next) => {
    try {
        const userId = req.userId;
        if (!userId) {
            throw new AppError('Unauthorized', 401);
 ...
 */
export const updateVendorBookingRequestStatus = async (req, res, next) => {
    try {
        const userId = req.userId;
        if (!userId) {
            throw new AppError('Unauthorized', 401);
        ...
        const transformedMessage = await bookingService.updateVendorBookingRequestStatus(userId, requestId, status);
