...
export class ReviewService {
   ...
   */
  async getReviewsByTarget(targetId: string, targetType: 'service' | 'vendor') {
    await dbConnect();
  ...
  async createListingReview(clerkUserId: string, reviewData: {
    ...
    if (!username) {
      throw new Error(
        'User profile incomplete. Missing username or name from Clerk.'
      );
    }
    ...
    if (!internalUser) {
      throw new Error(
        'Internal user not found. Please ensure your account is set up.'
      );
    }
   ...
   */
  async deleteListingReview(clerkUserId: string, reviewId: string) {
    await dbConnect();
