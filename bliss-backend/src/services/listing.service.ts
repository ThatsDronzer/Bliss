...
export class ListingService {
   ...
   */
  async getVendorListings(clerkId: string) {
    await dbConnect();
   ...
   */
  async getListingById(listingId: string, clerkId: string) {
    await dbConnect();
    const vendor = await Vendor.findOne({ clerkId });
    if (!vendor) {
      throw new Error('Vendor not found');
   ...
   * Create new listing
   */
  async createListing(clerkId: string, listingData: any) {
    await dbConnect();
   ...
   * Update listing
   */
  async updateListing(clerkId: string, updateData: any) {
    await dbConnect();
   ...
   */
  async toggleListingStatus(clerkId: string, listingId: string) {
    await dbConnect();
   ...
   */
  async addImages(clerkId: string, listingId: string, newImages: CloudinaryImage[]) {
    await dbConnect();
