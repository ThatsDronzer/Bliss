...
export class BookingService {
   ...
   */
  async getBookingStatus(userId: string, serviceId: string) {
    await dbConnect();
    ...
    const booking = await MessageData.findOne({
      ...
      .sort({ createdAt: -1 })
      .select(
        '_id bookingDetails.status paymentStatus createdAt listing.title bookingDetails.totalPrice'
      )
      .lean();
   ...
   * Cancel booking
   */
  async cancelBooking(userId: string, requestId: string) {
    await dbConnect();
  ...
  async createBookingMessage(bookingData: {
    ...
    if (
      ...
      throw new Error(
        'Missing required fields: userId, vendorId, listingId, address, bookingDate, bookingTime, totalPrice'
      );
    }
    ...
    const itemsSource =
      listing.items && listing.items.length > 0 ? listing.items : [];
    ...
    const newMessage = new MessageData({
      user: {
        id: booker.clerkId,
        name: bookerIsVendor
          ? (booker as any).ownerName
          : (booker as any).name,
        email: bookerIsVendor
          ? (booker as any).ownerEmail
          : (booker as any).email,
        phone: bookerIsVendor
          ? (booker as any).owner_contactNo?.[0]
          : (booker as any).phone,
   ...
   */
  async getVendorBookingRequests(userId: string, options: {
    status?: string;
    limit?: number;
    page?: number;
  }) {
   ...
   */
  async updateVendorBookingRequestStatus(
    userId: string,
    requestId: string,
