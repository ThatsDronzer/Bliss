...
export async function searchVendorsFromDb(query?: string, location?: string): Promise<any> {
	try {
		await dbConnect();
...
export async function getServiceByIdFromDb(serviceId: string): Promise<any> {
	try {
		await dbConnect();
		...
		const vendor = await Vendor.findById(service.owner).select(
		...
		return {
			...
			vendor: {
				...
				name: vendor.service_name || (vendor as any).name || 'Unknown Vendor',
