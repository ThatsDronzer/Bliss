...
import type { IVendor, ICreateVendorInput, IUpdateVendorInput } from '@models/vendor/vendor.model';

export async function getVendorByClerkIdFromDb(clerkId: string): Promise<IVendor | null> {
	try {
		await dbConnect();
...
export async function getVendorByIdFromDb(id: string): Promise<any> {
	try {
		await dbConnect();
		...
		return {
			id: (vendor as any)._id.toString(),
			name: (vendor as any).service_name,
			location: `${(vendor as any).service_address?.City || ''}, ${(vendor as any).service_address?.State || ''}`,
			rating: 4.5,
			category: (vendor as any).service_type,
			coverImage: '/images/vendors/default-cover.jpg',
			description: (vendor as any).service_description,
			shortDescription: (vendor as any).service_description?.slice(0, 150) + '...',
			services: services,
			packages: [],
			gallery: [],
			reviews: [],
			...
			contact: {
				phone: (vendor as any).service_phone,
				email: (vendor as any).service_email,
				whatsapp: (vendor as any).service_phone,
			},
...
export async function getVendorServicesFromDb(id: string): Promise<any[]> {
	try {
		await dbConnect();
...
export async function getVendorServicesForExploreFromDb(): Promise<any> {
	try {
		await dbConnect();
		...
		const vendorServices = vendors.map((vendor: any) => {
			const vendorListings = listings.filter(
				(listing: any) => listing.owner.toString() === vendor._id.toString()
			);

			const vendorServicesList = vendorListings.map((listing: any) => ({
				id: listing._id.toString(),
				name: listing.title,
				price: listing.price,
				category: listing.features[0] || 'Other',
...
export async function getVendorVerificationFromDb(clerkId: string): Promise<any> {
	try {
		await dbConnect();
...
export async function submitVendorVerificationInDb(verificationData: any): Promise<any> {
	try {
		await dbConnect();
		...
		if (!vendor) {
			...
			if (ownerEmail) vendor.ownerEmail = ownerEmail;
			if (businessName) vendor.service_name = businessName;
			if (businessEmail) vendor.service_email = businessEmail;
			if (businessPhone) vendor.service_phone = businessPhone;
			...
			if (businessDescription) vendor.service_description = businessDescription;
			if (establishedYear) vendor.establishedYear = establishedYear;
			if (businessType) vendor.service_type = businessType;
			...
			if (accountHolderName) vendor.accountHolderName = accountHolderName;
			...
			if (ownerAadhar) vendor.ownerAadhar = ownerAadhar;
...
export async function updateVendorByClerkIdInDb(clerkId: string, updateData: any): Promise<IVendor | null> {
	try {
		await dbConnect();
		...
		return updatedVendor as IVendor | null;
	} catch (error) {
