...
export async function getVendorListingsFromDb(clerkId: string): Promise<any> {
	try {
		await dbConnect();
...
export async function getListingByIdFromDb(listingId: string, clerkId: string): Promise<any> {
	try {
		await dbConnect();
		const vendor = await Vendor.findOne({ clerkId });
		if (!vendor) {
...
export async function createListingInDb(clerkId: string, listingData: any): Promise<any> {
	try {
		await dbConnect();
...
export async function updateListingInDb(clerkId: string, updateData: any): Promise<any> {
	try {
		await dbConnect();
		...
		if (imagesToDelete && Array.isArray(imagesToDelete) && imagesToDelete.length > 0) {
			...
			const imagesBeingDeleted = listing.images.filter((img: any) =>
		...
		if (itemImagesToDelete && Array.isArray(itemImagesToDelete) && itemImagesToDelete.length > 0) {
			const validItemImageIds = itemImagesToDelete.filter((id: string) => id && id.trim().length > 0);
		...
		if (category !== undefined) (listing as any).category = category;
		...
		if (tempIds.length > 0) {
			...
			const usedItemImageIds = items
				? items.map((item: any) => item.image?.public_id).filter((id: string) => id)
				: [];

			const allUsedIds = [...usedImageIds, ...usedItemImageIds];
			const imagesToCleanup = tempIds.filter((id: string) => !allUsedIds.includes(id));
...
export async function deleteListingFromDb(clerkId: string, listingId: string): Promise<string> {
	try {
		await dbConnect();
...
export async function toggleListingStatusInDb(clerkId: string, listingId: string): Promise<any> {
	try {
		await dbConnect();
...
export async function addImagesToListingInDb(clerkId: string, listingId: string, images: CloudinaryImage[]): Promise<any> {
	try {
		await dbConnect();
