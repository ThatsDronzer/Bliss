...
const verificationResults: ServiceVerification[] = [];

/**
 * Check if a file exists and contains expected function exports
 */
function verifyServiceFile(
	serviceName: string,
	filePath: string,
	expectedFunctions: string[]
): ServiceVerification {
...
function runVerification() {
	...
	verificationResults.push(
		verifyServiceFile(
			'AWS Service (SES)',
			'services/aws/aws.service.ts',
			['sendUsersEmail', 'sendBulkEmail']
		)
	);
	...
	verificationResults.push(
		verifyServiceFile(
			'Invoice Service',
			'services/invoice/invoice.ts',
			['createInvoicePDF', 'sendEmailInvoice']
		)
	);
	...
	verificationResults.push(
		verifyServiceFile(
			...
			'services/storage/uploadAsset.ts',
			['uploadAsset']
		)
	);
	...
	verificationResults.push(
		verifyServiceFile(
			...
			'services/payment/razorpay-gateway.service.ts',
